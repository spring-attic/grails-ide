...
/**
 * Working copy for groovy class files.  Allows access to the 
 * ModuleNode for class files if the source is available.
 * Copied from {@link ClassFileWorkingCopy} Groovy changes marked
...
public class GroovyClassFileWorkingCopy extends GroovyCompilationUnit {
    ...
    protected IBuffer openBuffer(IProgressMonitor pm, Object info) throws JavaModelException {
        ...
        if (buffer.getContents() != null) {
            ReflectionUtils.executePrivateMethod(BufferManager.class, "addBuffer", new Class<?>[] {IBuffer.class }, bufManager, new Object[] { buffer }); //$NON-NLS-1$
        }
        // GROOVY End
...
//                openWhenClosed(elementInfo, true, new NullProgressMonitor());
    ...
    public char[] getFileName() {
        return (this.classFile.getType().getElementName() + ".groovy").toCharArray(); //$NON-NLS-1$
    }
