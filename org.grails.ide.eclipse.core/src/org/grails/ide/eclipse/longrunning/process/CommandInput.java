 ...
 *
 */
class CommandInput extends Thread {
	...
	public void run() {
		...
		try {
			...
			while (!GrailsProcess.CONSOLE_EOF.equals(line)) {
				if (line.startsWith(GrailsProcess.CONSOLE_INPUT)) {
					toCommand.println(line.substring(GrailsProcess.PROTOCOL_HEADER_LEN));
				} else {
