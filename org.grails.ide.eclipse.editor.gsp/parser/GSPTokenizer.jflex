...
// gsp states
%state ST_GSP_CONTENT
// we don't yet support GSP comments
...
// GRAILS allow '/' in tag names, but last char cannot be a '/'
// [5.5] TagName ::= Name ('/' NameChar+)*
TagName = {Name} (\/ {NameChar}+)*
// GRAILS end
...
// GRAILS allow '/' in tag names
// [40]  STag  ::= '<' Name (S Attribute)* S? '>'
STag = (<{TagName}({S}{Attribute})*{S}?>)
// GRAILS end
...
// GRAILS allow '/' in tag names
// [44]  EmptyElemTag  ::= '<' TagName (S Attribute)* S? '/>'
EmptyElemTag = (<{TagName}({S}{Attribute})*{S}?\/>)
// GRAILS end
...
// GRAILS Allow the use of '/' in tag names
<ST_XML_TAG_NAME> {TagName} {
	if(Debug.debugTokenizer)
...
// GRAILS begin---handle gspScriptletEnd
<ST_GSP_CONTENT> {gspScriptletEnd} {
	if(Debug.debugTokenizer)
...
// GRAILS begin---gspScriptlet handling
{gspScriptletStart} {
